...
##
## ⚠️ 这里用 get_run_speed()（**含**被抓惩罚），是刻意的，别"顺手改成"干净速度。
##
## 因为 start_tier 是跑道上的**最低阶**（tier_at 只会往上加），
## 一旦开在某个阶位，你就**只能**面对那一阶的怪，跑不过也躲不开。
## 所以它必须等于「你**当前真的**跑得过的那一阶」：
##   用 get_run_speed()  → 被抓多了会开浅一点，但每一阶你都跑得过 ✓
##   用 get_clean_speed() → 惩罚期开得太深，怪比你快 → 必被抓 →
##                          惩罚更深 → 更跑不过 → **死循环** ✗
## 实测：本档 速度Lv5/跑鞋Lv3 干净速度 25.39、惩罚后 18.72。
## 干净速度会开到 T6（怪 22.3，跑不过），惩罚后速度才是 T5（怪 17.1，跑得过）。
##
## 唯一的例外是**下限**：绝不允许掉回 T1「草鸡」（用户明确说不要）。
## 兜底在 CreatureDB.start_tier_for() 里的 MIN_START_TIER。
